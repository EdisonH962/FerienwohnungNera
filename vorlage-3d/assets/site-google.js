/* Preserve {{FIRMA_KURZ}}'s own Places configuration. Never borrow another business's key. */
(() => {
  'use strict';
  ...
  if (!key || key.includes('HIER_EINF')) {
    note.textContent = 'Weitere Gästebewertungen finden Sie direkt auf Google.';
    note.hidden = false;
    return;
  }
  const safeUrl = value => {
  ...
  function render(place) {
    if (Number.isFinite(place.rating)) {
      ...
      document.getElementById('gCount').textContent = 'Google · ' + (Number.isFinite(place.userRatingCount) ? place.userRatingCount + ' Bewertungen' : 'Gästebewertung');
    }
    ...
    (place.photos || []).slice(0,config.maxPhotos || 10).forEach((photo,i) => {
      ...
      img.alt = '{{FIRMA_LANG}} – Foto ' + (i+1) + ' aus dem Google-Unternehmensprofil';
    ...
    note.textContent = (place.reviews || []).length
      ...
      : 'Weitere Gästebewertungen finden Sie direkt auf Google.';
    note.hidden = false;
  }
  let loaded = false;
  async function load() {
