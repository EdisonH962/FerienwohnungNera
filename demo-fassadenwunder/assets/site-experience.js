...
(() => {
  ...
  $$('.whatsapp-link').forEach(link => {
    link.href = 'https://wa.me/4960514973552?text=' + encodeURIComponent('Hallo, ich interessiere mich für die DAS FASSADEN-WUNDER und möchte gerne mehr erfahren.');
  });
  ...
  $('#mapShieldBtn')?.addEventListener('click', () => {
    ...
    iframe.src = shield.dataset.src; iframe.title = 'Karte Gelnhausen'; iframe.loading = 'lazy';
  ...
  const objekt = $('#bfObjekt'), leistung = $('#bfLeistung'), flaeche = $('#bfFlaeche'), name = $('#bfName');
  const error = $('#bfError'), summary = $('#bookingSummary'), status = $('#bookingStatus');
  function zusammenfassung() {
    const m = parseInt(flaeche.value, 10);
    summary.textContent = m > 0
      ? objekt.value + ' · ca. ' + m + ' m² · unverbindliche Anfrage'
      : 'Beschreiben Sie kurz Ihr Objekt.';
  }
  ...
  form.addEventListener('change', zusammenfassung);
  form.addEventListener('invalid', event => {
    event.target.setAttribute('aria-invalid', 'true');
    error.textContent = 'Bitte prüfen Sie die markierten Felder.';
  ...
  form.addEventListener('submit', event => {
    ...
    const lines = ['Guten Tag, ich möchte ein unverbindliches Angebot für eine Fassadenreinigung.', '',
      'Objekt: ' + objekt.value, 'Gewünschte Leistung: ' + leistung.value,
      'Fassadenfläche ca.: ' + (flaeche.value.trim() ? flaeche.value.trim() + ' m²' : 'unbekannt'), '',
      'Name: ' + name.value.trim(), 'E-Mail: ' + $('#bfEmail').value.trim(),
      'Telefon: ' + ($('#bfPhone').value.trim() || '-'), '', 'Anmerkungen:', $('#bfMessage').value.trim() || '-'];
    status.hidden = false;
    location.href = 'mailto:hallo@fassaden-wunder.de?subject=' + encodeURIComponent('Angebotsanfrage Fassadenreinigung') + '&body=' + encodeURIComponent(lines.join('\n'));
  });
  zusammenfassung();
