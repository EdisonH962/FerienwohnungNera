...
(() => {
  ...
  $$('.whatsapp-link').forEach(link => {
    link.href = 'https://wa.me/4960519777810?text=' + encodeURIComponent('Hallo, ich interessiere mich für die Besim Hoti Malermeisterfachbetrieb und möchte gerne mehr erfahren.');
  });
  ...
  $('#mapShieldBtn')?.addEventListener('click', () => {
    ...
    iframe.src = shield.dataset.src; iframe.title = 'Karte Gründau'; iframe.loading = 'lazy';
  ...
  const gewerk = $('#bfGewerk'), objekt = $('#bfObjekt'), ort = $('#bfOrt'), name = $('#bfName');
  const error = $('#bfError'), summary = $('#bookingSummary'), status = $('#bookingStatus');
  function zusammenfassung() {
    summary.textContent = gewerk.value + ' · ' + objekt.value
      + (ort.value.trim() ? ' · ' + ort.value.trim() : '') + ' · unverbindliche Anfrage';
  }
  ...
  form.addEventListener('submit', event => {
    ...
    const lines = ['Guten Tag, ich möchte ein unverbindliches Angebot.', '',
      'Gewerk: ' + gewerk.value, 'Objekt: ' + objekt.value,
      'Ort: ' + (ort.value.trim() || '-'), '',
      'Name: ' + name.value.trim(), 'E-Mail: ' + $('#bfEmail').value.trim(),
      'Telefon: ' + ($('#bfPhone').value.trim() || '-'), '', 'Vorhaben:', $('#bfMessage').value.trim() || '-'];
    status.hidden = false;
    location.href = 'mailto:info@malermeister-hoti.de?subject=' + encodeURIComponent('Anfrage Malermeister Hoti') + '&body=' + encodeURIComponent(lines.join('\n'));
  });
  zusammenfassung();
